export class SortingAlgorithmsService {
  ...
  public async sortBubble(array: number[]): Promise<number[]> {
  ...
  //Можливо додати ще змінну isSwapped і повертати масив якщо не було swap. Невелика оптимізація
  public async sortInsertion(array: number[]): Promise<number[]> {
  ...
  public async binaryInsertionSort(array: number[]): Promise<number[]> {
    for (let i = 1; i < array.length; i++) {
      let current = array[i];
  ...
  public async bubbleSort(array: number[]): Promise<number[]> {
    let n = array.length;
    let swapped;
    do {
      swapped = false;
  ...
  public async insertionSort(array: number[]): Promise<number[]> {
    for (let i = 1; i < array.length; i++) {
      let key = array[i];
      let j = i - 1;
  ...
  public async mergeSort(array: number[]): Promise<number[]> {
  ...
  public async quickSort(array: number[]): Promise<number[]> {
    if (array.length < 2) return array;
    ...
    return [
      ...(await this.quickSort(left)),
      ...middle,
      ...(await this.quickSort(right)),
    ];
  }

  public async heapify(arr: number[], n: number, i: number) {
    let largest = i;
    let left = 2 * i + 1;
    let right = 2 * i + 2;
  ...
  public async heapSort(array: number[]): Promise<number[]> {
    let n = array.length;
  ...
  public async selectionSort(array: number[]): Promise<number[]> {
  ...
  public async shellSort(array: number[]): Promise<number[]> {
    let n = array.length;
  ...
  public async countingSort(array: number[]): Promise<number[]> {
